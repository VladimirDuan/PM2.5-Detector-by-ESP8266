...
const char APwebPage1[] PROGMEM = "<!DOCTYPE HTML>\n"
                                  ...
                                  "<style type=\"text/css\">body {text-align:center;font-family: sans-serif;background-color: #E2F9FF;color: #B27246;font-size:2.0em;} a:link, a:visited, a:hover, a:active {color:#FFBA8A;}</style>\n"
                                  "</head>\n"
                                  "<body>\n"
                                  "<h1>PM2.5 Detector WiFi Setup</h1>\n<br>"
                                  "<table style=\"width:100%;border: 4px solid #FFBA8A;\"><tr>"
                                  "<th style=\"text-align:center;width:100%;\"><form action='/APsubmit' method='POST'><input type=\"text\" name=\"newssid\" id=\"formnewssid\" value=\"readonly\"><br><input type=\"text\" name=\"newpass\" value=\"\" size=\"32\" maxlength=\"64\"><br><input type=\"submit\" value=\" Save \"></form></th>"
                                  "<th style=\"text-align:left;width:40%;\">";
String APwebPage2 = "</th></tr></table>\n"
                    "<br><br><form action=\"/\" target=\"_top\"><input type=\"submit\" value=\"Refresh WiFi List\" style=\"font-size:30px\"></form>\n"
                    "<br><form action=\"" + String(restartcommand) + "\" target=\"_top\"><input type=\"submit\" value=\"RESET\" style=\"font-size:30px\"></form>\n"
                    "<br><form action=\"" + String(cleareepromcommand) + "\" target=\"_top\"><input type=\"submit\" value=\"Clear Data\" style=\"font-size:30px\"></form>\n"
                    "<b>- Version: 1." ESPWIFI2EEPROM_VERSION " -</b>\n"
                    "</body></html>";

String webPage1 = "<!DOCTYPE HTML>\n"
                  "<html><head><meta content=\"text/html;charset=utf-8\"><title>PM2.5 Detector</title>\n"
                  "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\">"
                  "</head>\n"
                  "<body>\n"
                  "<center>\n"
                  "<h1>PM2.5 Detector</h1>"
                  "<h2>Info Page</h2>\n";
...
void WifiConfig::handle_APsubmit()
{
  ...
  if (thenewssid != "") {
    ...
    if (EEPROM.commit())
      APwebstring = F("<h2><b><p>press RESET to boot into new wifi</b></h2><script>alert(\"Config saved. RESET to boot into new wifi!\");</script>\n");
    else
...
void WifiConfig::setupWiFiAP()
{
  ...
  macID.toUpperCase();
  String AP_NameString = F("PM2.5 Detector ");
  AP_NameString += macID;
...
void WifiConfig::handle_root() {
  ...
  webString = "ESP Chip ID: " + String(ESP.getChipId()) + "<br>ESP Flash Chip ID: " + String(ESP.getFlashChipId()) + "<br>ESP Flash Chip Size: " + String(ESP.getFlashChipSize()) + "<br>ESP Free Heap: " + String(ESP.getFreeHeap()) + "<br>Linked local WiFi: " + String(WiFi.SSID()) + "<br>Local WiFi RSSI: " + String(WiFi.RSSI()) + "<br>Local WiFi Password: BuNengGaoSuNi";
  server.send(300, "text/html", webPage1 + webString + webPage2);
  delay(100);
}

void WifiConfig::handle_gps() {
  gpsFlag = true;
  server.send(100, "text/html", "GPS Flag Recieved.");
}
...
void WifiConfig::initBasicHttpServer() {
  ...
  server.on(gpscommand, std::bind(&WifiConfig::handle_gps, this));
  server.begin();
